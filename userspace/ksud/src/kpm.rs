...
pub fn kpm_load(path: &str, args: Option<&str>) -> Result<()> {
    str_to_cstr(path, |p_path| unsafe {
        let p_args = match args {
            Some(a) => CString::new(a)?.as_ptr(),
            None => ptr::null(),
        };
        kpm_prctl(SUKISU_KPM_LOAD, p_path as _, p_args as _)?;
        println!("Success");
        Ok(())
    })?
}
...
pub fn kpm_unload(name: &str) -> Result<()> {
    str_to_cstr(name, |p| unsafe { kpm_prctl(SUKISU_KPM_UNLOAD, p as _, ptr::null()) })?;
    Ok(())
}
...
pub fn kpm_control(name: &str, args: &str) -> Result<i32> {
    str_to_cstr(name, |p_name| {
        str_to_cstr(args, |p_args| {
            unsafe { kpm_prctl(SUKISU_KPM_CONTROL, p_name as _, p_args as _) }
        })?
    })?
}
